...
<!--        如果是编辑车次的信息的时候车次编号不允许修改，如果是新增，该地方那可以修改-->
  ...
  setup() {
    ...
    watch(()=>train.value.start,()=>{
      if(Tool.isNotEmpty(train.value.start)){
        train.value.startPinyin=pinyin(train.value.start,{toneType:'none'}).replaceAll(" ","").toUpperCase();
      }
    },{immediate:true})
    watch(()=>train.value.end,()=>{
      if(Tool.isNotEmpty(train.value.end)){
        train.value.endPinyin=pinyin(train.value.end,{toneType:'none'}).replaceAll(" ","").toUpperCase();
      }
    },{immediate:true})
    const onAdd = () => {
      train.value = {};
